...
export default class IfConditionIsHolidayActionPo extends StrategyStepBasePo {
  async SetProperties(properties: string[]): Promise<this> {
    ...
    const dropdownCondition = await helpers.getElementWithinElement(conditionBuilder, By.className("condition-dropdown"), true, false, true, 5000);
    ...
    const dropdownValue = await helpers.getElementWithinElement(
      updatedConditionBuilder,
      By.css('[operation="IS_HOLIDAY_IN_COUNTRY"]'),
      true,
      false,
      true,
      5000
